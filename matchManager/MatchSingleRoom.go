...
var OneTeamMaxPlayerNum = 4
//单排匹配房间
//(根据排位分数分段)
type MatchSingleRoom struct {
...
///添加一个匹配队伍(根据段位)
func (this *MatchSingleRoom)AddOneMatchTeam(team face.IMatchTeam){
	score := team.GetTeamOwnerScore()
	if score >= 0 && score < 200{
		//青铜
		this.Lock()
		this.AllQingtongMatchTeams[this.QingTongNum] = team
		team.SetMatchRoomIndex(this.QingTongNum)
		this.QingTongNum++
		...
		team.SetMatchRoomIndex(this.BaiYinNum)
		this.BaiYinNum++
		...
		team.SetMatchRoomIndex(this.HuangJingNum)
		this.HuangJingNum++
		...
		team.SetMatchRoomIndex(this.BoJingNum)
		this.BoJingNum++
		...
		team.SetMatchRoomIndex(this.ZuanShiNum)
		this.ZuanShiNum++
...
func (this *MatchSingleRoom)EnterMatchBattleRoom(all map[int]face.IMatchTeam){
	this.MatchManager.EnterMatchBattleRoom(all)
}
func (this *MatchSingleRoom)RemoveOneMatchTeam(team face.IMatchTeam){
	score := team.GetTeamOwnerScore()
	if score >= 0 && score < 200{
		//青铜
		this.Lock()
