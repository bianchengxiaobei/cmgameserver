...
type CheckBuyCardHandler struct {
	GameServer face.IGameServer
}

func (handler *CheckBuyCardHandler) Action(session network.SocketSessionInterface, msg interface{}) {
	if innerMsg, ok := msg.(network.InnerWriteMessage); ok {
		if protoMsg, ok := innerMsg.MsgData.(*message.C2M_CheckBuyShopCard); ok {
